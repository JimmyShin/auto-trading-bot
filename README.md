...
- Engine: `class DonchianATREngine(persist_state: bool = True, initial_state: dict | None = None, risk_manager: RiskManager | None = None)` (houses risk/state pipeline while strategy selection happens via registry).
...
- Daily trading summary after trade closures (trades, wins/losses, win rate, PF, gross Â±%)
...
- no-trade anomalies (ï¿½ï¿½5 signals within 2h with zero filled trades)
