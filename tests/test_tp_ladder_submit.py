...
def test_tp_ladder_skip_reasons(dummy_exchange: DummyExchange, caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level("INFO", logger="donchian_bot")

    # zero_qty skip (qty rounded down by step size)
    dummy_exchange.set_precision(0.1, 1.0, 1.0)
    levels = [{"px": 100.0, "qty": 0.4}]
    place_tp_ladder(dummy_exchange, "ETH/USDT", "long", 1.0, 100.0, levels, dummy_exchange.market_precision("ETH/USDT"))
    ...
    place_tp_ladder(dummy_exchange, "ETH/USDT", "long", 0.01, 100.0, levels, dummy_exchange.market_precision("ETH/USDT"))
    dummy_exchange.set_precision(0.1, 0.001, 1.0)
...
def test_stop_refresh_preserves_tp_orders(dummy_exchange: DummyExchange, caplog: pytest.LogCaptureFixture) -> None:
    dummy_exchange.set_precision(0.1, 0.001, 1.0)
    caplog.set_level("INFO", logger="donchian_bot")
