...
def main():
    ...
    strategy_name = os.getenv("STRATEGY_NAME", STRATEGY_NAME)
    try:
        strat = get_strategy(strategy_name)
    except KeyError:
        fallback_name = "ma_cross_5_20"
        if strategy_name != fallback_name:
            _log_json(
                logger,
                logging.WARNING,
                {
                    "event": "strategy_missing",
                    "requested": strategy_name,
                    "fallback": fallback_name,
                },
            )
        strat = get_strategy(fallback_name)
        strategy_name = fallback_name
    strategy_state: Dict[str, Dict[str, Any]] = {}
    _log_json(
        logger,
        logging.INFO,
        {"event": "strategy_selected", "strategy": strategy_name},
    )
        ...
        try:
            ...
            for symbol in UNIVERSE:
                try:
                    ...
                    strategy_ctx = strategy_state.setdefault(symbol, {})
                    bar_payload = {
                        "symbol": symbol,
                        "history": df,
                        "close": close,
                    ...
                    try:
                        signal = strat.on_bar(bar_payload, strategy_ctx)
                    except Exception as sig_err:
                        _log_json(
                            logger,
                            logging.WARNING,
                            {
                                "event": "strategy_signal_error",
                                "symbol": symbol,
                                "error": str(sig_err),
